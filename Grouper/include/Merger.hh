...
string baseFileName;
string run_string;
pair<double, double> SiliconCalib[200][SIGNAL_MAX] = {{make_pair(0., 0.)}};
double SiliconCalib_Fermi[SIGNAL_MAX];
double SiliconCalib_GT[SIGNAL_MAX];
pair<double, double> SiPMCalib[200] = {{make_pair(0., 0.)}};
...
void InitHistograms()
{
  ...
  for (int i = 0; i < SIGNAL_MAX; i++)
  {
    if (IsDetectorSiliStrip(i))
    {
      ...
      HSiliconRun[i] = new TH2D(("HSiliconRun" + detectorName[i]).c_str(), ("HSiliconRun" + detectorName[i]).c_str(), n_bin_time, t_min, t_max, 5000, 0, 10000);
      ...
      ostringstream oss;
      oss << std::fixed << std::setprecision(1) << "Counts/" << (eSiliMax_cal-eSiliMin_cal)/eSiliN_cal << "keV";

      HSilicon[i] = new TH1D(("HStrip_" + detectorName[i]).c_str(), ("HStrip_" + detectorName[i]).c_str(), eSiliN_cal, eSiliMin_cal, eSiliMax_cal);
      HSilicon[i]->GetXaxis()->SetTitle("Energy [keV]");
      HSilicon[i]->GetYaxis()->SetTitle(oss.str().c_str());
      HSilicon[i]->GetXaxis()->CenterTitle();
      HSilicon[i]->GetYaxis()->CenterTitle();

      HSilicon_coinc[i] = new TH1D(("HStrip_coinc_" + detectorName[i]).c_str(), ("HStrip_coinc_" + detectorName[i]).c_str(), eSiliN_cal, eSiliMin_cal, eSiliMax_cal);
      HSilicon_coinc[i]->GetXaxis()->SetTitle("Energy [keV]");
      HSilicon_coinc[i]->GetYaxis()->SetTitle(oss.str().c_str());
      HSilicon_coinc[i]->GetXaxis()->CenterTitle();
      HSilicon_coinc[i]->GetYaxis()->CenterTitle();

      HSilicon_no_coinc[i] = new TH1D(("HStrip_no_coinc_" + detectorName[i]).c_str(), ("HStrip_no_coinc_" + detectorName[i]).c_str(), eSiliN_cal, eSiliMin_cal, eSiliMax_cal);
      HSilicon_no_coinc[i]->GetXaxis()->SetTitle("Energy [keV]");
      HSilicon_no_coinc[i]->GetYaxis()->SetTitle(oss.str().c_str());
  ...
  {
    HSiPMRun[GetDetectorChannel(i)] = new TH2D(("HSiPMRun_M" + to_string(i)).c_str(), ("HSiPMRun_M" + to_string(i)).c_str(), n_bin_time, t_min, t_max, eLowN / 10, eLowMin, eLowMax * 8);
  ...
  HSiPMRun_Channel_Unmatched = new TH2D("HSiPMRun_Channel_Unmatched", "HSiPMRun_Channel_Unmatched", n_bin_time, t_min, t_max, eLowN / 10, eLowMin, eLowMax * 8);
...
void InitFiles()
{
  ...
  if (Catcher == "Al")
  {
    RunNumbers = {57, 58, 59, 60, 61, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 77, 79, 112, 113, 114, 115, 116, 118};
...
inline void MakeSiPM_MultiplicityPlots()
{
  ...
  minimizer->SetFunction(functor);
  minimizer->SetLimitedVariable(0, "Calibration_OffSet", Par[0], 20, 0, 80);
  minimizer->SetFixedVariable(1, "Calibration", Par[1]);
  minimizer->SetFixedVariable(2, "Resolution_OffSet", Par[2]);
  minimizer->SetFixedVariable(3, "Resolution_SQRT", Par[3]);
  minimizer->SetFixedVariable(4, "Resolution_2", Par[4]);
  minimizer->SetLimitedVariable(5, "Threshold", Par[5], 5, 30, 70);
  minimizer->SetLimitedVariable(6, "Threshold_STD", Par[6], 10, 5, 16);
  ...
  for (int multiplicity = 1; multiplicity <= MAX_MULTIPLICTY; multiplicity++)
  {
    ...
    TreeHist_Conv_F_1D[multiplicity]->GetXaxis()->SetRangeUser(0, 8000);
    Ref_Hist_Multi_F[multiplicity]->GetXaxis()->SetRangeUser(0, 8000);
    TreeHist_Conv_F_1D[multiplicity]->Scale(Ref_Hist_Multi_F[MAX_MULTIPLICTY]->Integral() / TreeHist_Conv_F_1D[MAX_MULTIPLICTY]->Integral());
