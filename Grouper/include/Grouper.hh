...
vector<Signal> LAST_signals;  
double Channel[SIGNAL_MAX];
void SameNp1(TTreeReaderArray<Signal> &signals)
{
  ...
  // comparing channels of same detector
  for (int index = 0; index < signals.GetSize(); index++)
  {
    int current_label = signals[index].Label;
    if (IsDetectorSiliBack(current_label) || IsDetectorSiliStrip(current_label) || IsDetectorBetaHigh(current_label) || IsDetectorBetaLow(current_label))
    {
      cout << detectorName[signals[index].Label] << " : " << Channel[signals[index].Label] << " - " << signals[index].Channel << endl;
      if (Channel[signals[index].Label] == signals[index].Channel)
      {
  ...
  {
    int current_label = signals[index].Label;
    Channel[current_label] = signals[index].Channel;
    // if (IsDetectorSiliBack(current_label))
    // {
    //   signals[index].Channel = signals[index].Channel;// - 0.5 + gRandom->Rndm();
    //   signals[index].Time = signals[index].Time + (-0.5 + gRandom->Rndm()) * 8;
