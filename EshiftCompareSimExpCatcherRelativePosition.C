...
// ## Analyse simulation to match expeirmental data 
// keeping the eam posiution at MCP measurement
// keeping Position relative to the eam constant (previous minimization of the solid agle/ numer of counts)
// variation of catcher position independantly
...
int EshiftCompareSimExpCatcherRelativePosition()
{
    FLAG2025 = true;
    ...
    for (auto f : filename)
    {
        ...
        std::regex rgx("catcherx([-+]?[0-9]*\\.?[0-9]+)_catchery([-+]?[0-9]*\\.?[0-9]+)_catcherz([-+]?[0-9]*\\.?[0-9]+)");
        std::smatch match;
        if (std::regex_search(f, match, rgx) && match.size() == 4) 
        {
            string x_pos = to_string(stod(match[1]));
            string angle = "0.0";//to_string(stod(match[2]) - angle0);
            string y_pos = to_string(stod(match[2]));
            string z_pos = to_string(stod(match[3]) - z0);
            ...
            // {
                ...
                {
                    Chi2Dmapz[stod(z_pos)] = new TH2D(Form("Chi2D_z%s", z_pos.c_str()), Form("Chi2D z %s mm;X Position;Y Position;Chi2", z_pos.c_str()), 52, -6.5, 6.5, 52, -5.5, 5.5);
                }
